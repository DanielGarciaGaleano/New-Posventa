...
const addCumplimientoGen = async () => {
    console.log("Add prom examen");
        ...
        fetch(`${server}/filter/cumplimiento/general/`, {
            method: "POST",
...
const getTotal = async () => {
    fetch(`${server}/cumplimiento/general/promtotal`)
        .then(result => result.json())
        .then(data => {
            promedio_total = data;
            addTotal();
...
const addTotal = () => {
    document.querySelector('.iframe_content__total').innerHTML =''
    promedio_total.map((item) => {
        document.querySelector('.iframe_content__total').innerHTML += `
        <p class="total_titulo">PROGRESO TOTAL</p>
        <p class="procent1">${item. PromTotal}%</p>`
               

        if (item.PromTotal <= 49){
            document.querySelector(`.procent1`).setAttribute('promedio','rojo');
        }else if (item.PromTotal >49 && item.PromTotal<76){
            document.querySelector(`.procent1`).setAttribute('promedio','amarillo');
        }else if (item.PromTotal >= 76){
           document.querySelector(`.procent1`).setAttribute('promedio','verde');
        }
    });
}

const addPromedioTotal = async () => {
    console.log("Add prom examen");
        let filters = {
            "Usuario.Grupo": filtro_grupo,
            "Usuario.idUsuario": filtro_nombre,
        ...
        fetch(`${server}/filter/cumplimiento/general/prom/`, {
            method: "POST",
            body: JSON.stringify(filters),
            headers: { "Content-type": "application/json; charset=UTF-8" }
        })
