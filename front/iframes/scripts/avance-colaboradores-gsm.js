...
const getListdocumplimiento = async () => {
    fetch(`${server}/cumplimiento/cursos/nombre`)
        ...
        .then(data => {
            ...
            console.log(listado_cumplimiento);
...
const addListadoCumplimiento = () => {
    ...
    listado_cumplimiento.map((item,i) => {        
        ...
        <p>${i+1}</p>
        <p>${listado_cumplimiento[i].Razon_social}</p>
        <p>${listado_cumplimiento[i].Agencia}</p>
        <p>${listado_cumplimiento[i].Nombre}</p>
        <p>${listado_cumplimiento[i].CantCursos}</p>
        <p>${listado_cumplimiento[i].aprobados}</p>
        <p class="avance${i}">${listado_cumplimiento[i].Avance}%</p>`

        if (listado_cumplimiento[i].Avance <= 49){
            document.querySelector(`.avance${i}`).setAttribute('promedio','rojo');
        }else if (listado_cumplimiento[i].Avance >49 && listado_cumplimiento[i].Avance<76){
            document.querySelector(`.avance${i}`).setAttribute('promedio','amarillo');
        }else if (listado_cumplimiento[i].Avance >= 76){
...
const addCumplimientoGen = async () => {
    console.log("Add prom examen");
        ...
        fetch(`${server}/filter/cumplimiento/curso/nombre`, {
            method: "POST",
